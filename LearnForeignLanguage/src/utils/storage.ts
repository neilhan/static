...
export const loadState = (): AppState => {
  try {
        ...
        if (oldV1) {
            try {
                ...
                const migratedCollection: ConversationCollection = {
                    ...
                    targetVoiceURI: v1State.targetVoiceURI || v1State.targetLanguage || '',
                    targetLang: v1State.targetLang || 'en-US',
                    nativeVoiceURI: v1State.nativeVoiceURI || v1State.nativeLanguage || '',
    ...
    const collections = parsed.collections.map((c: any) => ({
        ...
        includePause: c.includePause ?? true, // Default to true if missing
        // Removed delayAfterTarget and autoPause from persistence logic (they will be dropped on save)
        srsData: c.srsData || {},
    }));
